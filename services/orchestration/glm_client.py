...
""".strip()
    ...
    user_prompt = (
        ...
        "3. 格式：类目字段为字符串数组，例如：[\"服饰\", \"女装\", \"连衣裙\"]。\n"
        "4. 如果只能判断到大类，就只输出一层，例如：[\"服饰\"]。\n"
        "5. 如果图片里同时有多个独立商品，选择最主体的一个商品打标。\n\n"
        "颜色标签规则：\n"
        "颜色字段为字符串数组。如果只有一个主颜色，数组长度为 1；如果有多个明显颜色，最多列出 5 个。\n\n"
        "材质/面料规则：\n"
        "字符串。单一材质直接写一种；如果商品明显由多种材质组成，可用英文逗号分隔，最多 3 种。\n\n"
        "款式特征规则：\n"
        "字符串。描述款式、功能或风格特点。如有多个显著特征，可用英文逗号分隔，最多 3 个。\n\n"
        "成色规则：\n"
        "单一字符串，描述商品新旧程度（如全新、九成新、八成新、七成新等）。\n\n"
        "置信度规则：\n"
